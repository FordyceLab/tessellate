...
#                                       scale_grad_by_freq=True)
        ...
        self.atom_linear = nn.ModuleList([nn.Linear(input_size, input_size, bias=False).to(device0) for layer in range(self.n_atom_conv)])
        self.res_linear = nn.ModuleList([nn.Linear(input_size, input_size, bias=False).to(device0) for layer in range(self.n_res_conv)])
